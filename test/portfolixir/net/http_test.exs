defmodule Portfolixir.Net.HttpTest do
  ...
  test "still decodes JSON under the cap" do
    req = Http.new(max_bytes: 1_000, allowed_hosts: @hosts)
    ...
    assert {:ok, %Req.Response{body: %{"a" => 1}}} = Http.get(req, url: @url, plug: plug)
  end
  ...
  test "refuses a declared Content-Length over the cap before reading" do
    ...
    assert {:error, %Http.BodyTooLarge{}} = Http.get(req, url: @url, plug: plug)
  end
  ...
  test "gives up at the deadline" do
    req = Http.new(max_bytes: 100, allowed_hosts: @hosts, deadline_ms: 50)
  ...
  test "returns adapter failures as values and ignores a malformed Content-Length" do
    ...
    assert {:error, %RuntimeError{message: "boom"}} =
             Http.get(req, url: @url, plug: fn _conn -> raise "boom" end)

    assert {:error, {:exit, :boom}} = Http.get(req, url: @url, plug: fn _conn -> exit(:boom) end)
    ...
    assert {:ok, %Req.Response{body: "ok"}} = Http.get(req, url: @url, plug: plug)
