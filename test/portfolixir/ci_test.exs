defmodule Portfolixir.CITest do
  ...
  test "the Compose deployment agrees with the runtime perimeter" do
    ...
    # E25 S7, G26 and its review round (S7E-5): the app takes the
    # companion's token whole through PORTFOLIXIR_API_TOKEN and names it
    # "mcp", so the journal names the companion; the operator's further
    # entries come from PORTFOLIXIR_API_TOKENS.
    [app_service] = Regex.run(~r/^  app:\n(?:    .*\n|\n)*/m, compose)

    assert app_service =~
             "PORTFOLIXIR_API_TOKEN: ${PORTFOLIXIR_API_TOKEN:?set PORTFOLIXIR_API_TOKEN in .env}"

    assert app_service =~ "PORTFOLIXIR_API_PRINCIPAL: mcp"
