...
- FR-42: **Exposure decomposition** (ladder level (b)): factor, sector and region exposure. Benchmark comparison is FR-9 and is not restated here.
...
**Sequencing dependency, binding on FR-43 through FR-46:** the audit-journal rollout (FR-28) is incomplete — Catalog and FX are armed; Portfolios, Classifications, Ledger and Imports still write unjournaled — and MCP *write* tools are deliberately blocked behind it so no agent can edit financial data without an audit trail. These four objects are agent-written by design, so **completing the rollout (#677) is a prerequisite, not a parallel nicety.**
...
- NFR-9: Mechanical scope backstop — the hard gates are backed by meta-tests in the invariant suite, each removable only in the same PR as its ADR and `AGENTS.md` amendment. *(Present in the PRD since 2026-06-12 and missing from this inventory until 2026-08-12; recorded here to close the drift.)* Since the identity gate the guarded set is Phase 3 sync, FR-5 XML intake, the permanent non-goals and the level-(d) backtesting gate, in place of the retired blanket analytics gate — a gate that lifts *partially* is exactly the kind a reader mistakes for lifted entirely.
...
| FR-37 | #665 | read ergonomics — sparse fieldsets, roll-up-only aggregates, server-side threshold filters. Ship-now, no gate. Supersedes FR-33's scope lock for this family only |
| FR-38 | #666 | `?since=` delta reads. Ship-now, no gate; the push half stays gated at B3.7 and must not ride the same story |
| FR-39, FR-40 | — | derived metrics per security / per view (ladder (a)). **Ungated by the ladder**, no issue yet — depends on the derived-value ADR (gate B3.2) for where the values live |
| FR-41, FR-42 | — | contribution analysis; factor/sector/region exposure (ladder (b)). **Ungated by the ladder**, no issue yet |
| FR-43 | — | policy rules as objects. **Gated: B3.6**, needs its own ADR (rules engine + rule-history retention). Blocked by #677 |
| FR-44 | — | security events as objects. **Gated: B3.4**; automatic population is B3.3. Blocked by #677. Distinct from ADR-0028 corporate actions |
| FR-45, FR-46 | — | thesis/conviction and prediction objects. Decided in principle by the identity gate; blocked by #677 |
| FR-47, FR-48 | — | calibration report (needs FR-46); rule evaluation (needs FR-43). Ladder (c) |
| NFR-9 | — | mechanical scope backstop — meta-tests in the invariant suite; guarded set revised 2026-08-12 |
