...
- FR-4: Portfolios partition the wealth space; every view/analytic can be scoped to one portfolio (UJ-6). Depots and cash accounts merge/rename/delete with transaction reassignment (#328). Moving them between portfolios (#327) is out of scope since ADR-0024 made portfolios internal (ADR-0050 §14).
...
- NFR-9: Mechanical scope backstop — the hard gates **must be** backed by meta-tests in the invariant suite, each removable only in the same PR as its ADR and `AGENTS.md` amendment. *(Entered the PRD on 2026-07-25 via #615 and was missing from this inventory until 2026-08-12; recorded here to close the drift.)* Since the identity gate the guarded set is Phase 3 sync, FR-5 XML intake, the permanent non-goals and the level-(d) backtesting gate, in place of the retired blanket analytics gate — a gate that lifts *partially* is exactly the kind a reader mistakes for lifted entirely. **Unbuilt:** of the three backstops the PRD names, only `mcp_dependency_allowlist_test` (ADR-0002) exists; there is no credential-schema test, no bank-domain-HTTP test and no non-goal test. This is a requirement, not an inventory. Its issue is **#885** (Sprint 16 Lane N, under #420).
...
| FR-23 | #338 (#588–#591); follow-on #907 | corporate actions — ADR-0028 accepted 2026-07-19; #338 repurposed as the E17 tracker. #588–#591 shipped; #338 closed 2026-07-31. **Follow-on filed 2026-09-24 (Sprint 16 branch opening) as #907**, needing a decision: merger and spin-off as ledger events with a ratio, ADR-0028 §4's follow-on, which ADR-0050 §15 deferred because they are not an identity repair. It has no parent, because the tracker that held the question (#338) is closed |
...
| FR-41, FR-42 | FR-41's side findings: #900, #901 | contribution analysis; factor/sector/region exposure (ladder (b)). **Ungated by the ladder**, no issue yet. **FR-41:** checked against ADR-0041 at Sprint 16's planning (plan D-7): **not** a thin extension — the category result has no period, no closed positions, no flows and no built view scope, and the engine that has them is the performance walk. It gets its own design-gate record in the shape of ADR-0047, written in Sprint 16 and signed by the merge of Sprint 17's planning PR; FR-41 itself is built in Sprint 17's batch (plan D-7). **The check's two side findings were filed at branch opening (2026-09-24), under #418:** #900 (ADR-0041 §5's slice two, realized result and income per category, had no issue) and #901 (ADR-0041's promised view scope for the category result was never built on the API and MCP side). Neither is FR-41's own issue, which follows its record. **FR-42 retired 2026-09-23** (backlog triage §3.6): sector and region exposure is the allocation breakdown over a classification tree (goals 10 and 12, shipped); factor exposure needs partial-weight assignments (advanced classifications, out of scope) or external factor data (B3.3). Nothing is left that is both allowed and new |
...
| NFR-9 | #885 | mechanical scope backstop — guarded set revised 2026-08-12. **Unbuilt requirement, not inventory:** only `mcp_dependency_allowlist_test` exists of the three named backstops. **Scheduled: Sprint 16 Lane N** (seven invariant files B1–B7, tests only), filed at branch opening (2026-09-24) as **#885** under #420 |
...
- **E3 — Account & portfolio lifecycle** — *phase 1, now.* Tracker **#417**
  (portfolio structure). Multiple portfolios usable end to end: switcher
  (#327; moving depots/accounts between portfolios is out of scope per
  ADR-0050 §14), merge/rename/delete with transaction reassignment (#328),
  import data quality and logos (#326). FR-4, FR-7.
  **Sprint 16:** the lifecycle merges (#328, and #608 for securities) under
  [ADR-0050](../../docs/decisions/0050-lifecycle-merges-under-a-reimport-contract.md),
  signed by the merge of the Sprint 16 planning PR; the importer half closes a
  live rename hazard on its own. Filed at branch opening (2026-09-24, Sprint 16
  Lane Z) under #417, where #328 and #608 already sat: the L2 issue **#884**
  (the re-import contract), and ADR-0050 §15's deferrals, each needing a
  decision — #902 (a tombstone for a source that notes or rule versions name),
  #903 (generalized identifier aliases for ISIN-less merges), #904 (a
  fail-closed probe for never-seen account identities), #905 (one-to-one
  matching on the pre-import #533 layer) and #906 (unmerge). The sixth,
  merger and spin-off, is #907 on FR-23's row.
- **E4 — Import completeness** — *phase 1, XML gated.* No tracker since the
  ...
  2026-09-23 triage. Lossless PP XML import stays **scope-gated**: it requires
  the AGENTS.md amendment plus an ADR before any implementation, and NFR-9's
  B6 backstop makes that mechanical. CSV/JSON v1 is shipped. FR-5.
...
- **E17 — Corporate actions as ledger events** — *phase 2/4 bridge.* Tracker
  **#338** (closed 2026-07-31); decision in **ADR-0028**. **Ledger events first,
  ...
  FR-23. Merger and spin-off, ADR-0028 §4's follow-on, are filed as #907
  (2026-09-24, deferred by ADR-0050 §15, needs a decision) without a parent.
...
- **E25 — Security hardening, second pass** — *cross-cutting, now.* The
  2026-09-24 whole-system review (`security-review-triage-2026-09-24.md`,
  adopted by the merge of the Sprint 16 planning PR), scheduled by the
  Sprint 16 plan as Lane H and Lane S. Tracker **#883**, filed at branch
  opening (2026-09-24, Sprint 16 Lane Z) with its eight group issues: #886
  (S1 credentials and sessions), #887 (S2 deployment and logging), #888 (S3
  outbound requests and provider data), #889 (S4 input bounds and cost, #868
  folded in), #890 (S5 import robustness), #891 (S6 the audit trail and
  integrity), #892 (S7 browser and agent-surface hygiene) and #893 (S8 CI
  supply chain). The follow-ups the triage names inside its rows are filed
  under the tracker and not in the batch's scope: #894 (dry-run deletes,
  G28), #895 (a Compose network split, F62), #899 (the architecture's
  Idempotency-Key, G31, for Sprint 17), and three that need a decision —
  #896 (a per-portfolio rule cap, F74), #897 (commit-ordered delta cursors,
  G06) and #898 (the database-role migration, T-6). The hotfix's issue (S0)
  is #879, closed by PR #880 ahead of the batch. The triage names each
  weakness in public from the moment the planning branch is pushed, so the
  public window runs from that push to the batch's merge (plan D-4). The
  review found the trust boundary E21 built still standing, and one runtime defect outside every gate: the
  release image's frozen base tag shipped an Erlang/OTP with published TLS
  client advisories, fixed by the hotfix of plan D-2 ahead of the batch. The
  rest is hardening with reach labels: credentials and sessions, the
  deployment files and log level, provider redirects and plausibility, input
  bounds and cost, import robustness, the audit trail's remaining silent
  paths, browser and agent-surface hygiene, and the CI supply chain. NFR-4,
  NFR-2, NFR-10. NFR-9 is Lane N's #885, under #420; S3's redirect fix lands
  first so that its B2 backstop pins the corrected chokepoint.
