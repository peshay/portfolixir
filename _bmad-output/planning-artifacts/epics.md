...
| FR-4 | #327, #328, #608 | portfolio switcher; merge/rename/delete. **Design gate: [ADR-0050](../../docs/decisions/0050-lifecycle-merges-under-a-reimport-contract.md)** (lifecycle merges under a post-merge re-import contract, risk-tier), signed by the merge of the Sprint 16 planning PR and built in Sprint 16's Lane L. #608, the security merge, is the same mechanism and carries FR-34's identity ladder across a merge (ADR-0050 §9). The "move between portfolios" clause of FR-4 is out of scope since ADR-0024 made portfolios internal (ADR-0050 §14) |
...
| NFR-9 | — | mechanical scope backstop — guarded set revised 2026-08-12. **Unbuilt requirement, not inventory:** only `mcp_dependency_allowlist_test` exists of the three named backstops. **Scheduled: Sprint 16 Lane N** (seven invariant files B1–B7, tests only; the issue is filed at branch opening under #420) |
...
- **E3 — Account & portfolio lifecycle** — *phase 1, now.* Tracker **#417**
  ...
  **Sprint 16:** the lifecycle merges (#328, and #608 for securities) under
  ...
  live rename hazard on its own. The L2 issue is filed at branch opening under
  #417, and #608 is attached to it.
...
- **E25 — Security hardening, second pass** — *cross-cutting, now.* The
  ...
  Sprint 16 plan as Lane H and Lane S. Its tracker and eight group issues
  (S1–S8) are filed at branch opening. The triage names each weakness in
  public from the moment the planning branch is pushed, so the public window
  runs from that push to the batch's merge (plan D-4). The review found the trust boundary
  E21 built still standing, and one runtime defect outside every gate: the
  ...
  NFR-2, NFR-10. NFR-9 is Lane N's, under #420; S3's redirect fix lands
