...
components:
  ...
  value-slot:
    ...
    pending: 'value unknown, a query is in flight — must be visually distinct from not-computable. [OPEN — owner decision pending] treatment P1–P4 in .working/loading-affordances.html'
    settling: 'value known, count-up running ~600ms — must be visibly not-yet-final. [OPEN — owner decision pending] treatment S1–S4 in .working/loading-affordances.html'
...
The slot reserves its final footprint in every state, so nothing reflows when a value lands, and uses tabular numerals throughout including mid-count. **[OPEN — owner decision pending]** the pending ornament (P1 typographic skeleton / P2 last known dimmed / P3 progressive reveal / P4 today's `…`) and the settling ornament (S1 accent underline / S2 digit shimmer / S3 pulsing marker / S4 colour resolve), rendered side by side in `.working/loading-affordances.html`. Closes on the owner's pick, one ID per section.

The allocation sunburst is a third case — eleven values landing over time rather than one — with its own three options (F1 sequential sweep / F2 rings inward-out / F3 fade-in-place) in the same file. **[OPEN — owner decision pending]**.
...
- **Hero** (dashboard centerpiece) — {components.hero}. [ASSUMPTION] The composition (value above curve, toggle on the frame's toolbar row) remains drafted, not owner-confirmed; [mockups/key-dashboard.html](mockups/key-dashboard.html) is the composition reference and the spine wins on conflict.
...
- **Mechanism lane, count-up — the CSS `@property` assumption is falsified.** The previous draft specified a pure-CSS counter. It cannot work: `counter()` renders an integer with no separators — `250000`, never `250.000,00`. Pure-CSS count-up and locale-formatted money are mutually exclusive, and money is always locale-formatted here. There is no CSS-only path to a counting money value.
  **[OPEN — owner decision pending]** the replacement mechanism. For scale: the repo already carries eight hand-written inline LiveView hooks (`AutoDismissToast`, `ChartCrosshair`, `ClassificationDnD`, `ColumnPrefs`, `PPImportDrop`, `PositionedMenu`, `SecuritySplitPane`, `SunburstTooltip`), so a small `requestAnimationFrame` + `Intl.NumberFormat` hook would be existing practice rather than a new dependency — which is a statement of cost, not a decision. The alternative of dropping count-up entirely remains on the table. Closes on the owner's call; this document does not decide it.
