...
|---|---|---|---|---|
| Overview | `/` | App open, brand link | Analysis home: total value + change, "Off target", data quality; the KPI strip since UX-DR2's 2026-09-14 amendment (unbuilt until #798) | built |
...
[ASSUMPTION] Owner desire, tentative: the Overview's metric cards become self-configurable. This spine keeps the layout future-friendly — cards are a flat, reorderable collection, no card depends on a sibling — and specifies **no** configuration mechanics. *Partly answered 2026-09-14:* UX-DR2's KPI strip is four fixed cells; whether they ever become configurable stays unspecified and tentative.
...
| Empty — no data at all | Overview | Dashed `.empty-state` well **replacing the total-value block** — the hero was retired by this session (UX-DR2) and cannot be what an empty state replaces. "Nothing here yet — import a PP export or record a transaction," linking to Import and Transactions. The KPI strip (UX-DR2 amended 2026-09-14) is absent in this state. |
...
| Error — the server's own answer | Any status the server answers itself: a form posted after its session expired, an unreadable or oversized request, an unknown address | The bare error page, no layout and no stylesheet: the status and its reason in the page's language (`403 · Zugriff verweigert`, `403 · Forbidden`), the language read from the page's choice, else the locale cookie, then the browser's language, then English (E25, board 11). The same line for every status, 404 and 500 included. A designed error page is its own decision. |
...
*Built state re-verified 2026-09-15 (#798):* the line is built as specified — `dashboard_live.ex` renders one `{components.data-note}` (`data-role="data-quality-line"`) at the highest severity present, only when a count is non-zero, each count linking to the securities list pre-filtered to that set (`?dq=stale_quote`, `filter[]=asset_class:is_nil`, `?dq=missing_logo`). The pre-filtered URL the 2026-08-05 text recorded as blocked exists since #651 (URL-addressable `dq` and filter params on `/securities`), closed by #688 on 2026-08-14.
...
*Amended 2026-09-14 (owner decision — pick C2-B of the 2026-09-12 review; built by #798):* the Overview carries a **four-cell KPI strip** under the value card — TTWROR with its period (IRR as the sub-line), cash quote with its amount, last booking with its date and kind, quote freshness. Four fixed cells, not configurable cards; each cell **states its basis** and navigates to the surface that owns the figure (Wealth for the return and the cash quote, Transactions for the last booking, the securities list pre-filtered to stale quotes for freshness — the link the data-quality line carries today). Three things the mockup left open are decided here, so #798 does not decide them silently: **(1)** the strip's TTWROR is the 1Y figure and the Overview has no period control (the per-surface subsets table is unchanged); the value card's sub-line keeps the YTD change and drops the cash quote, which the strip cell now carries. **(2)** The freshness cell's value is the date of the newest stored quote across held positions — the thin read the State Patterns → *Stale data* row already owes, with its API/MCP field and its basis stated there; its sub-line carries "n veraltet" only when n > 0, linking where the data-quality line links, and with n = 0 the cell shows the date alone — a freshness fact, not the all-clear badge this rule forbids. The data-quality line keeps its rule (one line, N > 0 only) and keeps the stale finding: the cell is the fact, the line is the finding. **(3)** The "Off target" card (UX-DR21; "Needs attention" until #718) keeps its rows; each row also carries a drift bar around zero beside the text, decorative (`aria-hidden`) — sign, colour and the direction word stay the accessible channels (UX-DR7). The strip is the Overview's own block, not the Wealth band repeated (the 2026-09-12 one-band-per-area extension concerns sibling tabs), and it is absent in the empty state. This returns three of the 2026-06-13 metrics under a new shape and adds a fourth, freshness, which that set did not have; top drift does not return as a cell, because the "Off target" card *is* that figure. The hero stays retired and `key-dashboard.html` stays stale. Appearance: `DESIGN.md` → Components, written by #798. *(The 2026-08-05 paragraphs above describe the data-quality line as unbuilt and #651 as open; #688 closed #651 on 2026-08-14 and `dashboard_live.ex` renders `data-role="data-quality-line"` with pre-filtered links, so those paragraphs were stale — corrected 2026-09-15 by #798.)*
...
| `/securities` | Sidebar `nav-securities`; the Overview's data-quality line and the KPI strip's quotes cell (pre-filtered) | yes |
...
bucket cell **A** (chips with a scope line). All nine land their anatomy in
`DESIGN.md` → Components with their stories; C3-A's appearance is already
decided in `DESIGN.md`'s 2026-09-12 amendment (two-line phone rows).
