...
>
> **Gate.** `status: draft` lifts to `final` when the items below are decided. `DESIGN.md` carries the visual half of the same gate.
>
> | Open item | Where | What closes it |
> |---|---|---|
> | Categorical colour is unreconciled — a closed token set, free-form per-category hex, and accent tints for stacked segments cannot all be true | IA → Per-instrument income; `DESIGN.md` → Category and series colour | Its own decision gate; it touches user-set data, not styling. Blocks the per-instrument income story. |
> | The danger tint fails contrast, so problem-severity notes are unbuildable | `DESIGN.md` → the danger-tint gate | One ruling between three measured resolutions. Blocks the Wealth data-quality story. |
> | The Overview data-quality line has no pre-filtered URL to link to | UX-DR2 | URL-addressable filter params on `/securities`, or an issue that states the shortfall. |
> | The securities detail pane's tabs are second-level by analogy, not by decision | Component Patterns → Tab system | One line from the owner or the designer; the log names only Cash flow. |
> | Note-level tone was never fixed by a decision | State Patterns → Data note — note | Confirmation that neutral is {colors.text-muted} on {colors.bg-muted}, as the component now assumes. |
...
| UX-DR6 | Touch targets ≥ {spacing.touch-target} under `pointer: coarse` | here |
...
**Call sites:** Alignment inventory → UX-DR17 — the six Wealth data-quality conditions with their severities, plus the alert-class sweep.
...
**Story boundary.** Two: the Wealth list (six rows, one surface, self-contained) and the alert-class sweep (many surfaces, mechanical). Both are blocked on the icon-set story for the three glyphs, and the Wealth list's *problem* row is additionally blocked on the danger-tint gate in `DESIGN.md` → Colors.
