...
×80.5 as "my money times 80"?

**Evidence.**
- Owner's own analysis (2026-07-10, external LLM session): max-period TTWROR
  +7,953% (×80.5) vs. the honest pair — net invested ~298k, current ~1,536k →
  real-money multiple ×5.15, money-weighted IRR ~35% p.a. over 13 years.
- #545: long-period TTWROR is additionally inflated by trade-price re-pricing
  of unquoted holdings — the ×80 is not even a clean TTWROR. Sequencing with
  the metric work is mandatory (don't explain a number, then change it).
...
- Candidate KPI set: cumulative + annualized TTWROR (exists) · **net invested
  ...
  realized/unrealized (the owner's Bitcoin-FIFO analysis shows the appetite).
...
**The question.** External read-only broker sync (e.g. Andi's comdirect
tooling: OAuth+photoTAN pull of depot/positions/postbox, settlement-PDF
...
together against the live instance — no repo changes expected:

1. **Lombard/credit line**: model the comdirect −167.6k as a cash account with
   liquidity role `credit_line`; the 136k Tagesgeld as `reserve` (roles exist
   in `portfolio_accounts_live.ex`). Then the cash line stops being cosmetic.
2. **Negative holdings** (Beone −500 etc.): identify via the future #570
