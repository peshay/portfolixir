...
const toolDefinitions: ToolDefinition[] = [
  ...
  tool("portfolixir.cashflow.realized_gains", "Realized gains roll-up", "The Cash-flow Realized-gains facet's roll-up (issue #724): FIFO-matched realized P&L across ALL securities and portfolios, grouped by each sale's close date into a year -> month matrix. FX basis (D-1): each sale converts to the base currency via the EUR hub at the rate stored on ITS OWN close date; a sale with NO stored rate for that day is excluded from every converted total and named in `excluded` (count + security names) — never converted at a neighbouring date's rate, never silently dropped. The payload carries computation_basis (series, window, reference, gaps) and conversion_note; financial values are Decimal strings. Optional limit keeps the newest years of the matrix (default 100, at most 1000); computation_basis.window names the cut when years were dropped. Since issue 807 the payload also carries `trades` — the closed round-trips themselves, newest close first, each with security_id and security_name, open_date, close_date, holding_period_days, quantity, basis, proceeds, realized_pnl_abs/_pct in the trade's own currency and realized_base in the base currency — and `summary`: realized_total, hit_rate (the share of closed trades whose result is strictly positive; a break-even trade counts as a miss) and average_holding_period_days. The three figures are derived from the SAME converted trades as the matrix, so an excluded sale is in none of them, and with no closed trades hit_rate and average_holding_period_days are null rather than 0 — the average of nothing is not zero. `limit` cuts only the matrix's years; the figures and the list always read the full history. computation_basis.summary states all of this in the payload.", yearsLimitSchema, yearsLimitZ),
  ...
  tool("portfolixir.portfolios.valuation", "Value portfolio", "Live valuation of a portfolio: market values, actual weights per position, plus the base-currency portfolio total, cash balances and the cash quote (use this, not holdings.list, for base-currency totals). The valued/price_source flags mark stale or unpriceable positions, each position's price_date says when its price is from, stale_priced_count counts the quoted positions whose quote is older than the data-quality threshold, retired holdings left out (the operator's remedy is the security's is_retired flag, which the performance walk keys on, or a quote sync), newest_quote_date is the newest stored quote date across the quoted, non-retired positions (null when none is quote-priced), and unvalued_reason distinguishes no_price (nothing resolves) from missing_fx (a native latest_price/price_currency is known but no stored FX path reaches the base currency). Pass an optional view (a view id) to scope the result to the holdings matching that bucket view; the response then echoes the active view. Pass include_positions=false (FR-37) for a roll-up-only read — totals, cash balances and cash quote without the per-position rows; the response states positions_included.", {
    type: "object",
    additionalProperties: false,
    required: ["portfolio_id"],
    properties: {
  ...
  tool(
    ...
    "Answer 'would I have done better keeping what I had?': values the snapshot's frozen holdings buy-and-hold over the real stored quote history (daily, EUR-hub FX) and compares against the scope's real TTWROR since the as-of date. Gross, price-return only (v1); all financial values are Decimal strings; the response is self-describing (basis, base_currency) and lists securities excluded for missing quotes/FX under gaps. The two sides are NOT on one basis and the payload says so: the frozen side pays nothing ever, while the real TTWROR is net of the fees and taxes actually booked. So it also returns real_ttwror_before_costs (the same walk with window TRADE fees/taxes reclassified as external flows), transaction_costs (their total in base_currency) and cost_recovery.state - recovered | partly_recovered (ahead before costs, not yet after, with the outstanding gap) | not_recovered (behind even before costs, so the costs are not the reason) | not_comparable (the frozen side has no value at the as-of date - e.g. an all-cash snapshot - so there is nothing to be ahead of; costs still report). Read the three together: real_ttwror_before_costs alone flatters. basis.costs_removed / basis.costs_kept name which kinds left the return - standalone fee/tax bookings and dividend withholding deliberately stay in, because the frozen holder would have paid those too.",
