...
This foundation slice adds domain behavior for account relationships, transaction recording, derived
cash balances and derived security positions. It does not add public API endpoints.

## Decision

Public API endpoints are deferred for this slice.

The domain model is now more useful, but the API boundary should wait until the read models and
security assumptions are stable enough to expose intentionally.
...
- Read models should stabilize after derived balances and positions have been exercised in the
  UI/domain layer.
