---
layout: docs
title: "ADR-0020: SOLL target plans belong to a view"
description: Decision that allocation target weights (the SOLL plan) and the cash target become a per-view plan keyed by (view, classification), with view_id NULL as the portfolio-wide "Gesamt" plan, so each view carries its own coherent 100% steering plan instead of a single global target set.
---

# ADR-0020: SOLL target plans belong to a view
...
views, and #444 scoped the analytics so the **IST** (actual) side of the
SOLL/IST allocation runs against a chosen view (valuation/allocation/risk accept
...
The **SOLL** side did not move with it. Target weights live in
...
**global** — they have no view dimension. Once the IST became view-scoped this
...
2. **A latent bug.** Under any view, `Portfolixir.Portfolios.Allocation` still
   ...
   target; `top_level_target_sum` sums every root target.) So the SOLL side is
...
**A SOLL target plan belongs to a view.**
...
- A view may carry its **own** plan per classification, **or none**. With no
  plan, the portfolio page shows that view's allocation **IST-only** (no SOLL,
...
- **Editing** lives on the **classifications page** with an added view selector
  ...
  pre-selected, so a plan is never edited fully blind to the IST.
...
3. **Editing on the classifications page** (with a view selector), not the
   portfolio page; the portfolio page stays the read/IST surface and links into
   the editor.
...
- **Trade-off (accepted):** editing a plan on the classifications page is one
  step removed from the IST it is measured against; the deep-link from the
...
- Net worth and the IST stay exactly as they are; this decision only changes
...
and #335 — the SOLL/IST drift mechanics themselves are unchanged.
