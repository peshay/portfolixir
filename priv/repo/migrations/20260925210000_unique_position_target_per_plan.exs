defmodule Portfolixir.Repo.Migrations.UniquePositionTargetPerPlan do
  ...
  naming each plan and security: which row stays is the operator's choice,
  never the migration's. Remove all but one row of each pair (the plan
  editor, or `DELETE /api/v1/portfolios/:portfolio_id/position_targets/
  :category_id/:security_id`) and migrate again. Creating an index rewrites
  no row and fires no row trigger, so the journal is untouched.
  """
  use Ecto.Migration
  ...
  defp duplicate_message(duplicates) do
    pairs =
      Enum.map_join(duplicates, "; ", fn [plan_id, security_id, category_ids] ->
    ...
    "portfolio_targets holds more than one position row for one security in a plan: " <>
      pairs <>
      ". A plan carries one position row per security (E25 S6, G13); remove all but one " <>
      "row of each pair in the plan editor or through DELETE " <>
      "/api/v1/portfolios/:portfolio_id/position_targets/:category_id/:security_id, " <>
      "then migrate again."
  end
end
