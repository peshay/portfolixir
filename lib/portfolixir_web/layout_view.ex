      ...
      <body>
        ...
        <script id="live-view-client-script">
          (function () {
            ...
            Hooks.PerfCrosshair = {
              mounted: function () { this.setup(); },
              updated: function () { this.teardown(); this.setup(); },
              destroyed: function () { this.teardown(); },
              setup: function () {
                try {
                  this.payload = this.readPayload();
                  this.svg = this.el.querySelector("svg.perf-chart");
                  if (!this.svg || !this.payload.points || this.payload.points.length === 0) return;

                  this.line = document.createElement("div");
                  this.line.className = "chart-crosshair";
                  this.line.hidden = true;
                  this.el.appendChild(this.line);

                  this.tip = document.createElement("div");
                  this.tip.className = "chart-tooltip";
                  this.tip.hidden = true;
                  this.tip.setAttribute("role", "status");
                  this.el.appendChild(this.tip);

                  var self = this;
                  this.onMove = function (e) { self.move(e); };
                  this.onLeave = function () { self.hide(); };
                  this.svg.addEventListener("pointermove", this.onMove);
                  this.svg.addEventListener("pointerleave", this.onLeave);
              ...
              readPayload: function () {
                var node = this.el.querySelector("script[data-perf-payload]");
                if (!node) return { points: [], view: { width: 720, height: 180 } };
                try { return JSON.parse(node.textContent); }
                catch (_) { return { points: [], view: { width: 720, height: 180 } }; }
              },
              move: function (event) {
                var p = this.payload;
                ...
                if (r.width === 0) return;
                var view = p.view || { width: 720, height: 180 };
                var sx = (event.clientX - r.left) * (view.width / r.width);
                var idx = this.nearest(p.points, sx);
                var pt = p.points[idx];
                var frame = this.el.getBoundingClientRect();
                var cssX = (r.left - frame.left) + pt[3] * (r.width / view.width);

                this.line.style.left = cssX + "px";
                this.line.style.top = (r.top - frame.top) + "px";
                this.line.style.height = r.height + "px";
                this.line.hidden = false;

                this.tip.innerHTML =
                  '<div class="chart-tooltip__date"></div>' +
                  '<div class="chart-tooltip__price"></div>' +
                  '<div class="chart-tooltip__price"></div>';
                var rows = this.tip.querySelectorAll("div");
                rows[0].textContent = pt[0];
                rows[1].textContent = pt[1];
                rows[2].textContent = pt[2];
                this.tip.hidden = false;

                var tw = this.tip.offsetWidth;
                var left = cssX + 12;
                if (left + tw > frame.width) left = cssX - tw - 12;
                this.tip.style.left = Math.max(0, left) + "px";
                this.tip.style.top = (r.top - frame.top + 8) + "px";
              },
              nearest: function (points, x) {
                var best = 0, bd = Infinity;
                for (var i = 0; i < points.length; i++) {
                  var d = Math.abs(points[i][3] - x);
                  if (d < bd) { bd = d; best = i; }
                }
                return best;
              },
