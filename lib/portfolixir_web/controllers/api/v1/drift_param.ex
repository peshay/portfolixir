defmodule PortfolixirWeb.Api.V1.DriftParam do
  ...
  detour, through the shared finite-decimal parser
  (`Portfolixir.Input.BoundedDecimal.parse/1`, E25 S4, G15); anything that is
  not a full clean finite Decimal is `{:error, :min_drift}` and the caller
  answers `422`. Both the allocation read and the
