defmodule PortfolixirWeb.Api.V1.MergeController do
  @moduledoc """
  The lifecycle merges over the API (ADR-0050 §7, §8, §10, §12; #328): a cash
  account into another, and a depot into another.

  `GET /api/v1/cash_accounts/:id/merge_preview?target_id=` and `GET
  /api/v1/securities_accounts/:id/merge_preview?target_id=` are reads: the
  preview of merging the account or depot into `target_id`, every figure for
  both values of `collapse_key_equal`, and the `plan_digest` the apply takes.

  `POST /api/v1/cash_accounts/:id/merge` and `POST
  /api/v1/securities_accounts/:id/merge` with `{"target_id", "plan_digest",
  "collapse_key_equal"}` merge under the token: **201** with the merge
  ...
  writes nothing and names its code in `errors.code`:
    ...
    * **409** — a guard (for a cash account `same_account`, `not_live`,
      ...
      `position_buckets_mismatch` — each with `errors.guards`),
