  ...
  @entries [
    %{
      ...
      parameters: [
        ...
        "Every /api/v1 write is journaled under the name of the token entry the presented bearer token matched, as actor_label on GET /api/v1/journal (portfolixir.journal.list), never under anything the request says: API tokens may be configured as PORTFOLIXIR_API_TOKENS name=token entries, each with the same full authority, and PORTFOLIXIR_API_TOKEN stays the unnamed default, journaled with actor_label null as before; the Compose deployment gives the companion's token the name mcp (E25 S7, G26, the architecture's FU-6)",
