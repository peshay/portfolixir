  ...
  @entries [
    %{
      ...
      parameters: [
        ...
        "GET /api/v1/securities_accounts/:id/merge_preview?target_id= (portfolixir.securities_accounts.merge_preview) is a read: 200 with plan_digest, source and target (cash_account_id, bucket_ids — the default buckets —, former_names, transaction_count), guards, internal_transfers (the security transfers between the two), key_equal_pairs, choice_required, position_buckets (per security the source holds or overrides: both effective bucket sets, both overrides and the action none, carry, drop_redundant, drop_unheld or clear_target), former_names (appended, not_kept, after), outcome_by_collapse_key_equal with \"false\" and \"true\" (transaction_count, moved_transaction_ids, deleted with reason internal_transfer or collapsed_duplicate, positions — per security the source holds, source and target before and after, each quantity, cost_basis, avg_cost and realized_result —, rounding_differences per split where the combined position rounded once differs from the two rounded apart, cash_accounts a collapse changes with balance_before and balance_after, flow_changes a collapse moves into a later balance anchor of such a cash account, kind absorbed, as in the cash merge preview, other_depots a collapsed transfer with a third depot changes, with securities_account_id, security_id, quantity_before and quantity_after; each key-equal pair names both depot legs, securities_account_id and counter_securities_account_id) and positions_basis, the computation basis of those figures; every quantity and decimal a string. A pair a guard forbids answers 409 with errors.code (same_account, not_live, portfolio_mismatch, buckets_mismatch, position_buckets_mismatch naming the positions) and errors.guards, an unknown source 404, a source already merged 409 already_merged with errors.merged_into, a missing target_id 422 (ADR-0050 §7, §8, §10)",
        ...
        "GET /api/v1/securities/:id, /cash_accounts/:id and /securities_accounts/:id (portfolixir.securities.get) of a row a merge took away answer 404 with errors.merged_into {kind, id}, as does every route under /api/v1/securities/:security_id/ (quotes, trades, metrics, notes, events, logo, reads and writes) for a merged-away security — the row its history lives on now, following every later merge to the live end — and a detail naming both; an id no merge names answers the plain 404 as before (ADR-0050 §12)",
